...
public class ZooKeeperInitializer {
  ...
  void initializeConfig(final InstanceId instanceId, final ZooReaderWriter zoo) {
    try {
      ...
      zoo.putPersistentData("/", EMPTY_BYTE_ARRAY, ZooUtil.NodeExistsPolicy.SKIP);
  ...
  void initialize(final ServerContext context, final boolean clearInstanceName,
    ...
    // setup the instance
    zoo.putPersistentData(Constants.ZTABLES, Constants.ZTABLES_INITIAL_ID,
        ZooUtil.NodeExistsPolicy.FAIL);
    zoo.putPersistentData(Constants.ZNAMESPACES,
        NamespaceMapping
